                ...
                .filter_map_fields(|field| {
                    ...
                    match field.purpose {
                        Purpose::SubExpr => Some(format!(
                            "(rule ((= true (is-inv-Expr loop_id expr1)) \
                            {br} (= false (is-inv-Expr loop_id expr2)) \
                            {br} (= expr2 {ctor_pattern}) \
                            {br} (= expr1 {var})) \
                            {br}((boundary-Expr loop_id expr1)){ruleset})\n"
                        )),
                        _ => None,
                    }
                })
...
fn loop_invariant_detection1() -> Result<(), egglog::Error> {
    ...
    let check = "
        ...
        (check (boundary-Expr id1 (Get (Arg id1) 0)))
        (check (boundary-Expr id1 (Add (Num id1 1) (Get (Arg id1) 0)) ))  
    ";
