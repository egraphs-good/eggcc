...
impl TreeProgram {
    ...
    pub(crate) fn dag_with_arg_types(&self) -> TreeProgram {
        let mut checker = TypeChecker::new(self);
        checker.dag_add_arg_types()
    }

    pub fn dag_with_arg_types_and_cache(&self) -> (TreeProgram, TypeCache) {
        let mut checker = TypeChecker::new(self);
        let prog = checker.dag_add_arg_types();
...
impl Expr {
    ...
    #[allow(dead_code)]
    pub(crate) fn dag_with_arg_types(self: RcExpr, input_ty: Type, output_ty: Type) -> RcExpr {
        let prog = self.to_program(input_ty.clone(), output_ty.clone());
        let mut checker = TypeChecker::new(&prog);
        let (ty, new_expr) = checker.dag_add_arg_types_to_expr(self.clone(), &input_ty);
        assert_eq!(
            ty, output_ty,
    ...
    pub(crate) fn func_dag_with_arg_types(self: RcExpr) -> RcExpr {
        match self.as_ref() {
            Expr::Function(name, in_ty, out_ty, body) => RcExpr::new(Expr::Function(
                ...
                out_ty.clone(),
                body.clone()
                    .dag_with_arg_types(in_ty.clone(), out_ty.clone()),
            )),
...
impl<'a> TypeChecker<'a> {
    ...
    pub(crate) fn dag_add_arg_types(&mut self) -> TreeProgram {
        TreeProgram {
            entry: self.dag_add_arg_types_to_func(self.program.entry.clone()),
            functions: self
                .program
                .functions
                .iter()
                .map(|expr| self.dag_add_arg_types_to_func(expr.clone()))
                .collect(),
        }
    }

    pub(crate) fn dag_add_arg_types_to_func(&mut self, func: RcExpr) -> RcExpr {
        match func.as_ref() {
            Expr::Function(name, in_ty, out_ty, body) => {
                let (expr_ty, new_body) = self.dag_add_arg_types_to_expr(body.clone(), in_ty);
                assert_eq!(
                    expr_ty, *out_ty,
                    "Expected return type to be {:?}. Got {:?}",
                    out_ty, expr_ty
    ...
    pub(crate) fn dag_add_arg_types_to_expr(&mut self, expr: RcExpr, arg_ty: &Type) -> (Type, RcExpr) {
        ...
        let (res_ty, res_expr) = match expr.as_ref() {
            ...
            Expr::Top(TernaryOp::Write, left, right, state) => {
                let (lty, new_left) = self.dag_add_arg_types_to_expr(left.clone(), arg_ty);
                let (rty, new_right) = self.dag_add_arg_types_to_expr(right.clone(), arg_ty);
                let (_sty, new_state) = self.dag_add_arg_types_to_expr(state.clone(), arg_ty);
            ...
            Expr::Uop(op, inner) if op.types().is_some() => {
                ...
                let (ity, new_inner) = self.dag_add_arg_types_to_expr(inner.clone(), arg_ty);
                assert_eq!(
                    ity, expected_inner,
            ...
            Expr::Bop(BinaryOp::Print, inner, state) => {
                let (_ity, new_inner) = self.dag_add_arg_types_to_expr(inner.clone(), arg_ty);
                let (_sty, new_state) = self.dag_add_arg_types_to_expr(state.clone(), arg_ty);
                (
                    base(statet()),
            ...
            Expr::Get(child, index) => {
                let (cty, new_child) = self.dag_add_arg_types_to_expr(child.clone(), arg_ty);
            ...
            Expr::Alloc(amount, state, ty) => {
                let (aty, new_amount) = self.dag_add_arg_types_to_expr(amount.clone(), arg_ty);
                let (_sty, new_state) = self.dag_add_arg_types_to_expr(state.clone(), arg_ty);
            ...
            Expr::Call(string, arg) => {
                let (aty, new_arg) = self.dag_add_arg_types_to_expr(arg.clone(), arg_ty);
                let func = self
                    .program
                    .get_function(string)
            ...
            Expr::Single(arg) => {
                let (Type::Base(basety), new_arg) = self.dag_add_arg_types_to_expr(arg.clone(), arg_ty)
                else {
            ...
            Expr::Switch(integer, branches) => {
                let (ity, new_integer) = self.dag_add_arg_types_to_expr(integer.clone(), arg_ty);
                ...
                for branch in branches {
                    let (bty, new_branch) = self.dag_add_arg_types_to_expr(branch.clone(), arg_ty);
            ...
            Expr::If(pred, then, else_branch) => {
                let (pty, new_pred) = self.dag_add_arg_types_to_expr(pred.clone(), arg_ty);
                let Type::Base(BaseType::BoolT) = pty else {
                    panic!("Expected bool type. Got {:?}", pty)
                };
                let (tty, new_then) = self.dag_add_arg_types_to_expr(then.clone(), arg_ty);
                let (ety, new_else) = self.dag_add_arg_types_to_expr(else_branch.clone(), arg_ty);
                assert_eq!(
                    tty, ety,
            ...
            Expr::Let(input, body) => {
                let (ity, new_input) = self.dag_add_arg_types_to_expr(input.clone(), arg_ty);
                let (bty, new_body) = self.dag_add_arg_types_to_expr(body.clone(), &ity);
                (bty, RcExpr::new(Expr::Let(new_input, new_body)))
            }
            Expr::DoWhile(inputs, pred_and_outputs) => {
                let (ity, new_inputs) = self.dag_add_arg_types_to_expr(inputs.clone(), arg_ty);
                let Type::TupleT(in_tys) = ity.clone() else {
                    panic!("Expected tuple type. Got {:?}", ity)
                };
                let (pty, new_pred_and_outputs) =
                    self.dag_add_arg_types_to_expr(pred_and_outputs.clone(), &ity);
            ...
            Expr::InContext(assumption, body) => {
                let (bty, new_body) = self.dag_add_arg_types_to_expr(body.clone(), arg_ty);
                (
                    bty,
