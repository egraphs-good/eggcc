...
fn rvsdg_basic_odd_branch() {
 ...
 .exit:
  ret res;
    ...
    let actual: egglog::ast::Expr = actual.to_egglog_expr();
    let actual = egglog::ast::Command::Action(egglog::ast::Action::Let("actual".into(), actual));
    const EGGLOG_PROGRAM: &str = r#"
(let expected (Project 0 
    (Gamma 
        (Node (PureOp (lt 
            (Project 0 
                (Theta 
                    (Node (PureOp (lt (Node (PureOp (add (Arg 1) (Node (PureOp (Const (const) (IntT) (Num 1))))))) 
                                      (Arg 2)))) 
                    (vec-of (Node (PureOp (Const (const) (IntT) (Num 0)))) 
                            (Node (PureOp (Const (const) (IntT) (Num 0)))) 
                            (Arg 0)) 
                    (vec-of (Node (PureOp (add (Arg 0) (Arg 1)))) 
                            (Node (PureOp (add (Arg 1) (Node (PureOp (Const (const) (IntT) (Num 1))))))) 
                            (Arg 2)))) 
            (Node (PureOp (Const (const) (IntT) (Num 5))))))) 
        (vec-of 
            (Project 0 
                (Theta (Node (PureOp (lt (Node (PureOp (add (Arg 1) (Node (PureOp (Const (const) (IntT) (Num 1))))))) (Arg 2)))) 
                       (vec-of (Node (PureOp (Const (const) (IntT) (Num 0)))) (Node (PureOp (Const (const) (IntT) (Num 0)))) (Arg 0)) 
                       (vec-of (Node (PureOp (add (Arg 0) (Arg 1)))) (Node (PureOp (add (Arg 1) (Node (PureOp (Const (const) (IntT) (Num 1))))))) (Arg 2))))) 
        (vec-of (VO (vec-of (Arg 0))) 
                (VO (vec-of (Node (PureOp (mul (Arg 0) (Node (PureOp (Const (const) (IntT) (Num 2)))))))))))))"#;
    ...
    egraph
        .process_commands(vec![actual], egglog::CompilerPassStop::All)
        .unwrap();
    egraph
