use crate::greedy_dag_extractor::{get_root, prune_egraph, DefaultCostModel};
...
struct FunctionExtractionData {
    root_cid: ClassId,
    extraction: TigerExtraction,
}

/// Core Tiger extractor struct (split from monolithic implementation).
pub struct TigerExtractor {
    pub(crate) serialized: EGraph,
    pub(crate) tiger: TigerEGraph,
}

impl TigerExtractor {
    pub fn new(serialized: &EGraph, func: &str) -> Self {
        let function_root = serialized.nid_to_cid(&get_root(serialized, func)).clone();
        let pruned = prune_egraph(serialized, function_root, &DefaultCostModel {});
        let tiger = build_tiger_egraph(&pruned);
        Self {
            serialized: pruned,
            tiger,
        }
...
pub fn tiger_extractor_extract(serialized: &EGraph, functions: &[String]) -> TigerExtractionResult {
    ...
    for func in functions {
        let extractor = TigerExtractor::new(serialized, func);
        let (mut logs, outcome) = extractor.extract_single_function(func);
        debug_lines.append(&mut logs);

        let Some(FunctionExtractionData {
            root_cid,
            extraction,
        }) = outcome
        else {
        ...
        let root_enode_index = extraction
            .root_index
            .map(|ridx| extraction.nodes[ridx].enode_index);
        if let Some(enode_idx) = root_enode_index {
            chosen_enodes.insert(root_cid.clone(), enode_idx);
        }

        extractions.insert(root_cid.clone(), extraction);
        ...
        function_roots.insert(func.clone(), root_cid);
    }
...
impl TigerExtractor {
    fn extract_single_function(&self, func: &str) -> (Vec<String>, Option<FunctionExtractionData>) {
        let mut debug_lines = Vec::new();

        let Some(root_cid) = self.function_body_root(func) else {
            debug_lines.push(format!("func={} missing_root", func));
            return (debug_lines, None);
        };
