...
pre_entry:
  br label %for.cond.1
for.cond.1:
  %i_1 = phi i64 [ %i_2, %for.incre ], [ 2, %pre_entry ]
  %sum_1 = phi i64 [ %sum_4, %for.incre ], [ 1, %pre_entry ]
  %ii_0 = mul i64 %i_1, %i_1
  %comp_0 = icmp sgt i64 %ii_0, %input
  br i1 %comp_0, label %for.end, label %for.body
for.body:
  %qut_0 = sdiv i64 %input, %i_1
  %mpt_0 = mul i64 %qut_0, %i_1
  %dif_0 = sub i64 %input, %mpt_0
  %comp1_0 = icmp eq i64 %dif_0, 0
...
if.body:
  ...
  br label %for.incre
for.incre:
  %sum_4 = phi i64 [ %sum_3, %if.body ], [ %sum_1, %for.body ]
  %i_2 = add i64 %i_1, 1
  br label %for.cond.1
for.end:
  %comp2_0 = icmp eq i64 %sum_4, %input
  br i1 %comp2_0, label %if.success, label %if.failure.end
if.success:
  br label %if.failure.end
if.failure.end:
  %result_2 = phi i64 [ 0, %if.success ], [ 1, %for.end ]
  call void @print_int(i64 %result_2)
